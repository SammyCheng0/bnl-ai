...
def calculate_padding(original_width, original_height, target_width, target_height):
    ...
    print("target width: ", target_width, ", target height: ", target_height)

    if original_aspect_ratio > target_aspect_ratio:
        # Pad height
        new_height = (original_height*original_aspect_ratio)/ target_aspect_ratio
        print("new height: ",new_height)
        padding_height = (new_height - original_height) / 2
        print("padding_height: ",new_height)
        padding_width = 0
        ...
        # Pad width
        new_width = (original_width*original_aspect_ratio)/ target_aspect_ratio
        print("new width: ",new_width)
        padding_width = (new_width - original_width) / 2
        print("padding_width: ",new_width)
        padding_height = 0
...
class SideViewDataset(Dataset):
    def __init__(self, image_folder, output_size, label_file=None, rotate=False, infer=False, debug=False):
        ...
        if infer:
            ...
            # bounding_box = self.labels[['bbox_tl-x', 'bbox_tl-y', 'bbox_br-x', 'bbox_br-y']].copy()
            # print(bounding_box)
    ...
    def __getitem__(self, idx):
        ...
        if not self.infer:
            ...
            # bounding_box = valid_keypoints[['bbox_tl-x', 'bbox_tl-y', 'bbox_br-x', 'bbox_br-y']].copy()
            # bbox_tl = self.labels[-2]
            # print(bbox_tl)
            # bbox_br = self.labels[-1]
            # print(bbox_br)

            # valid_keypoints = valid_keypoints[['bbox_tl-x', 'bbox_tl-y', 'bbox_br-x', 'bbox_br-y']].drop()
            ...
            # # transformed_image = transformed_image.crop((valid_keypoints[-2], valid_keypoints[-1], valid_keypoints[-4], valid_keypoints[-3]))
            # # bounding_box = self.labels[['bbox_tl-x', 'bbox_tl-y', 'bbox_br-x', 'bbox_br-y']].copy()
            # # print(bounding_box)
            # # self.labels.drop(labels=['bbox_tl-x', 'bbox_tl-y', 'bbox_br-x', 'bbox_br-y'], axis=1, inplace=True)
        ...
        padding_width, padding_height = calculate_padding(*transformed_image.size, self.output_size[1], self.output_size[0])
