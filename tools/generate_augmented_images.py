...
# for images, _, _, _ in data_loader:
#     # Flatten (batch_size, C, H, W) to (C, N)
#     pixels = images.view(3, -1)
#     mean_sum += pixels.mean(dim=1)
#     std_sum += pixels.std(dim=1)
#     num_batches += 1
...
class PoseDataset(Dataset):
    ...
    def __init__(self, image_folder, resize_to, heatmap_size, label_file=None, rotate=False, scale=False, motion_blur=False, brightness=False, contrast=True, sharpness=False, gamma= False, mean=None, std=None, save_augmented=True, save_dir='augmented_images'):
        """
        Args:
        ...
        image_filenames = [img for img in os.listdir(self.image_folder) if img in csv_filenames]
    ...
    def __getitem__(self, idx):
        ...
        # PoseDataset.py, line 187 (modify this block):
        matching_rows = self.labels[self.labels['filename'] == img_name]

        if matching_rows.empty:
            raise IndexError(f"No label found for image '{img_name}' in annotation CSV.")
        ...
        #     # Construct filename
        #     base_filename = self.image_filenames[idx].split('.')[0]
        ...
        # if self.save_augmented:
        #     to_pil = ToPILImage()
        #     pil_image = to_pil(transformed_image.clone())  # Use clone to avoid modifying the tensor
        #     base_filename = self.image_filenames[idx].split('.')[0]
        #     save_path = os.path.join(self.save_dir, f"{base_filename}_augmented.jpg")
        #     pil_image.save(save_path)
        ...
        if self.save_augmented:
            to_pil = ToPILImage()
            pil_image = to_pil(transformed_image.clone())

            base_filename = os.path.splitext(self.image_filenames[idx])[0]
            aug_type = 'contrast'  # or 'brightness', etc.
            new_filename = f"{base_filename}_{aug_type}.jpg"
            save_path = os.path.join(self.save_dir, new_filename)
            pil_image.save(save_path)

            print(f"Saved augmented image: {save_path}")

            # Copy the full label row and update only the filename
            # new_row = self.labels_df.iloc[idx].copy()
            new_row = self.labels.iloc[idx].copy()

            new_row['filename'] = new_filename
            self.new_labels.append(new_row)
    ...
    def save_augmented_labels_csv(self, output_csv_path):
        if not self.new_labels:
            print("No new labels to save.")
            return
        new_labels_df = pd.DataFrame(self.new_labels)
        all_labels_df = pd.concat([self.labels, new_labels_df], ignore_index=True)
        all_labels_df.to_csv(output_csv_path, index=False)
        print(f"Saved augmented labels to {output_csv_path}")
...
if __name__ == "__main__":
    # Set paths
    # image_folder = r"dataset/train"
    # label_file = r"dataset/annotations_final.csv"
    image_folder = "dataset/train"
    label_file   = "dataset/annotations_final.csv"
    ...
    dataset.save_augmented_labels_csv("augmented_labels.csv")
